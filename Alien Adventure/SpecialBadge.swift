...
class SpecialBadge: Badge {
  ...
  override init(requestType: UDRequestType) {
    ...
    animateBadge(Int(arc4random_uniform(3)))
  }
  ...
  func animateBadge(badgeAnimation: Int){
    switch badgeAnimation.hashValue{
    case BadgeAnimation.GrowAndShrink.hashValue:
      growAndShrink()
    case BadgeAnimation.Rotate.hashValue:
      rotate()
    case BadgeAnimation.Shake.hashValue:
      shake()
    default:
      break
    }
